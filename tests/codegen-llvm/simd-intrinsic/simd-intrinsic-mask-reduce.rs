...
pub unsafe fn reduce_any_32x2(x: mask32x2) -> bool {
    ...
    // CHECK: [[C:%_?[0-9]+]] = call[[ADDRSPACE]] i1 @llvm.vector.reduce.or.v2i1(<2 x i1> [[B]])
    // CHECK: ret i1 [[C]]
    simd_reduce_any(x)
}
...
pub unsafe fn reduce_all_32x2(x: mask32x2) -> bool {
    // CHECK: [[A:%[0-9]+]] = lshr <2 x i32> %{{x|0}}, {{<i32 31, i32 31>|splat \(i32 31\)}}
    // CHECK: [[B:%[0-9]+]] = trunc <2 x i32> [[A]] to <2 x i1>
    // CHECK: [[C:%_?[0-9]+]] = call[[ADDRSPACE]] i1 @llvm.vector.reduce.and.v2i1(<2 x i1> [[B]])
    // CHECK: ret i1 [[C]]
    simd_reduce_all(x)
}
...
pub unsafe fn reduce_any_8x16(x: mask8x16) -> bool {
    ...
    // CHECK: [[C:%_?[0-9]+]] = call[[ADDRSPACE]] i1 @llvm.vector.reduce.or.v16i1(<16 x i1> [[B]])
    // CHECK: ret i1 [[C]]
    simd_reduce_any(x)
}
...
pub unsafe fn reduce_all_8x16(x: mask8x16) -> bool {
    // CHECK: [[A:%[0-9]+]] = lshr <16 x i8> %{{x|0}}, {{<i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7, i8 7>|splat \(i8 7\)}}
    // CHECK: [[B:%[0-9]+]] = trunc <16 x i8> [[A]] to <16 x i1>
    // CHECK: [[C:%_?[0-9]+]] = call[[ADDRSPACE]] i1 @llvm.vector.reduce.and.v16i1(<16 x i1> [[B]])
    // CHECK: ret i1 [[C]]
    simd_reduce_all(x)
}
