...
type UninitFatPointer = core::mem::MaybeUninit<&'static dyn core::fmt::Debug>;
...
pub unsafe fn volatile_load(a: *const u16) -> u16 {
    // CHECK: [[TEMP:%.+]] = load volatile i16, ptr[[ADDRSPACE]] %a
    // CHECK-SAME: align 2{{,|$}}
...
pub unsafe fn volatile_load_bool(a: *const bool) -> bool {
    // CHECK: [[TEMP:%.+]] = load volatile i8, ptr[[ADDRSPACE]] %a
    // CHECK-SAME: align 1{{,|$}}
...
pub unsafe fn volatile_load_array(a: *const [u16; 8]) -> [u16; 8] {
    // CHECK-NOT: alloca
    // CHECK: [[TEMP:%.+]] = load volatile i128, ptr[[ADDRSPACE]] %a,
    // CHECK-SAME: align 2{{,|$}}
    // CHECK: store i128 [[TEMP]], ptr[[ADDRSPACE]] %_0,
    // CHECK-SAME: align 2{{,|$}}
    // CHECK-NEXT: ret void
    intrinsics::volatile_load(a)
}
...
pub unsafe fn volatile_load_fat(a: *const UninitFatPointer) -> UninitFatPointer {
    ...
    // CHECK: [[TEMP:%.+]] = load volatile [[INT:i32|i64|i128]], ptr[[ADDRSPACE]] %a,
    // CHECK-SAME: align [[ALIGN]]{{,|$}}
    // CHECK: store [[INT]] [[TEMP]], ptr[[ADDRSPACE]] [[ALLOCA]],
    // CHECK-SAME: align [[ALIGN]]{{,|$}}

    // CHECK: [[T0:%.+]] = load ptr[[ADDRSPACE]], ptr[[ADDRSPACE]] [[ALLOCA]], align [[ALIGN]]
    // CHECK: [[T1:%.+]] = getelementptr inbounds i8, ptr[[ADDRSPACE]] [[ALLOCA]]
    // CHECK: [[T2:%.+]] = load ptr[[ADDRSPACE]], ptr[[ADDRSPACE]] [[T1]], align [[ALIGN]]
    // CHECK: [[P1:%.+]] = insertvalue { ptr[[ADDRSPACE]], ptr[[ADDRSPACE]] } poison, ptr[[ADDRSPACE]] [[T0]], 0
    // CHECK: [[P2:%.+]] = insertvalue { ptr[[ADDRSPACE]], ptr[[ADDRSPACE]] } [[P1]], ptr[[ADDRSPACE]] [[T2]], 1
    // CHECK: ret { ptr[[ADDRSPACE]], ptr[[ADDRSPACE]] } [[P2]]
    intrinsics::volatile_load(a)
}
...
pub unsafe fn unaligned_volatile_load_fat(a: *const UninitFatPointer) -> UninitFatPointer {
    ...
    // CHECK: [[TEMP:%.+]] = load volatile [[INT]], ptr[[ADDRSPACE]] %a,
    // CHECK-SAME: align 1{{,|$}}
    // CHECK: store [[INT]] [[TEMP]], ptr[[ADDRSPACE]] [[ALLOCA]],
    // CHECK-SAME: align [[ALIGN]]{{,|$}}
...
pub unsafe fn unaligned_volatile_store(a: *mut u16, b: u16) {
    // CHECK: store volatile i16 %b, ptr[[ADDRSPACE]] %a, align 1
...
pub unsafe fn unaligned_volatile_store_pair(a: *mut (u16, u16), b: (u16, u16)) {
    // CHECK: store volatile i16 %b.0, ptr[[ADDRSPACE]] %a, align 1
    // CHECK: [[TEMP:%.+]] = getelementptr inbounds i8, ptr[[ADDRSPACE]] %a, {{i16|i32|i64}} 2
    // CHECK: store volatile i16 %b.1, ptr[[ADDRSPACE]] [[TEMP]], align 1
    intrinsics::unaligned_volatile_store(a, b)
}
...
pub unsafe fn unaligned_volatile_store_array(a: *mut [u16; 7], b: [u16; 7]) {
    ...
    // CHECK-NOT: memcpy
    // CHECK: call[[ADDRSPACE]] void @llvm.memcpy{{.+}}(ptr[[ADDRSPACE]] align 1 %a, ptr[[ADDRSPACE]] align 2 %b, {{i16|i32|i64}} 14, i1 true)
    // CHECK-NOT: memcpy
    // CHECK: ret void
    intrinsics::unaligned_volatile_store(a, b)
}
